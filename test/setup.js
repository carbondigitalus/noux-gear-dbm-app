...
const getUserDataPath = function() {
  switch (process.platform) {
    ...
    default:
      throw new Error(
        `Unknown userDataPath path for platform ${process.platform}`
      );
  }
};
...
const setupApp = function(app) {
  ...
  app.client.addCommand('selectSection', function(section) {
    return this.click(`button[data-section="${section}"]`)
      .pause(100)
  ...
  app.client.addCommand('expandDemos', function() {
    return this.execute(function() {
      for (const demo of document.querySelectorAll('.demo-wrapper')) {
  ...
  app.client.addCommand('auditSectionAccessibility', function(section) {
    ...
    return this.selectSection(section)
      ...
      .then(function(audit) {
        if (audit.failed) {
          throw Error(
            `${section} section failed accessibility audit\n${audit.message}`
          );
        }
      });
  });
